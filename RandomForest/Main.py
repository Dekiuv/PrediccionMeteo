    ...
    with col1:  # Columna 1 (parte izquierda)
        ...
        if st.button("Realizar predicción"):
            with st.spinner('Realizando predicciones...'):
                ...
                feature_columns = ["temp_min", "humidity", "pressure", "solar_radiation", "precipitation", "temp_range"]  # Columnas de características
                target_columns = ["temp_min", "temp_max", "precipitation"]  # Columnas objetivo a predecir
                y_pred, y_pred_limited, mae, mse, r2, accuracy = simulador.entrenar_y_predecir(df_procesado, feature_columns, target_columns, dias)  # Entrenar y predecir
            ...
            with st.expander("Información predicción"):
                ...
                st.write(f"Accuracy en prueba: {accuracy:.2f}%")  # Mostrar el accuracy en %

            # Columna 2 (parte derecha)
            with col2:
                graf.grafica(y_pred_limited, dias)  # Mostrar la gráfica de la predicción
                with st.expander("Predicciones"):
                    # Mostrar las predicciones con temperatura mínima, máxima y precipitación
                    st.subheader("Predicciones con temperatura mínima, máxima y precipitación")
                    for index, row in y_pred_limited.iterrows():
                        # Mostrar los datos de cada día
                        st.write(f"Dia{index+1} | Temp Mín: {row['temp_min']:.2f}°C | Temp Máx: {row['temp_max']:.2f}°C | Precipitación: {row['precipitation']:.2f}mm")
